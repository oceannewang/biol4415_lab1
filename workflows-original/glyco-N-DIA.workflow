...
skyline.skyline-mods-mode=N-glyco
...
speclibgen.easypqp.extras.max_glycan_qval=0.01
...
workflow.description=Runs MSFragger-Glyco and PTM-Shepherd to build a spectral library from DDA n-glyco data, then quantifies DIA data using the library. Uses glycan FDR filtering from PTM-Shepherd when building the library. Load both DDA and DIA data on the workflow tab and set the Data Type to "DIA-quant" for all DIA files.
