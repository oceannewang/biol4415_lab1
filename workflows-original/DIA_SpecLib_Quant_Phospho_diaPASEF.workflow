# Workflow: DIA_SpecLib_Quant_Phospho_diaPASEF
...
diatracer.mass-defect-filter=false
diatracer.mass-defect-offset=0.1
diatracer.rf-max=500
diatracer.run-diatracer=true
...
msfragger.table.var-mods=15.9949,M,true,1; 42.0106,[^,true,1; 79.96633,STY,true,3; -17.0265,nQnC,false,1; -18.0106,nE,false,1; 4.025107,K,false,2; 6.020129,R,false,2; 8.014199,K,false,2; 10.008269,R,false,2; 0.0,site_10,false,1; 0.0,site_11,false,1; 0.0,site_12,false,1; 0.0,site_13,false,1; 0.0,site_14,false,1; 0.0,site_15,false,1; 0.0,site_16,false,1
...
ptmprophet.cmdline=NOSTACK KEEPOLD STATIC FRAGPPMTOL\=10 EM\=1 NIONS\=b STY\:79.96633,M\:15.9949,n\:42.0106 MINPROB\=0.7
...
workflow.description=Workflow for diaPASEF phosphorylation data. Peptides are identified directly from DIA data (.d files labelled as 'DIA' or 'DIA-Lib') using diaTracer and MSFragger. Met oxidation, N-term Acetyl and Phospho on STY are specified as variable modifications, with up to 2 missed cleavages allowed. PSM rescoring with MSBooster and validation with Percolator, followed by site localization with PTM-Prophet, and protein inference with ProteinProphet. Spectral library is built using EasyPQP, with data filtered to 1% FDR at the PSM, peptide, and protein levels. DIA-NN is used to extract quantification from each run annotated as \u2018DIA\u2019 using that library. PTM-Prophet localization information is propagated to the DIA-NN output files, which are then further processed to generate site-level reports. NOTE\: diaTracer needs to be run only once. If .diatracer.mzML files already exist (located in the folder next to the original .d files), load them and annotate as 'DDA' type. Load the original diaPASEF .d files and annotate as 'DIA-Quant'. This will avoid redoing the conversion from .d to diatracer.mzML files.
