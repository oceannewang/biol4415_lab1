...
3. Copy necessary data into the `./data` folder (fasta, modified workflow, mzMl, manifest.tsv, etc.)
    - Move the .mzML files from the `./mzMLFiles` folder and fasta files from the `./fastaFiles` in to the `./data` folder or just mount the ./data folder from the OneDrive biol4415 folder to the docker container in the `./run.sh` 
